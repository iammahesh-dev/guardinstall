//! Sandboxer with seccomp filter that ALLOWS execve but blocks sockets
...
/// Apply seccomp filter that blocks sockets but ALLOWS execve
fn apply_seccomp_filter() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // BPF program that blocks socket(AF_INET) but allows everything else
    #[repr(C)]
    struct sock_filter {
        code: u16,
        jt: u8,
    ...
    // This filter:
    // 1. Checks if syscall is socket (41 on x86_64)
    // 2. If socket, checks if arg0 == AF_INET (2)
    // 3. If AF_INET, blocks with EPERM
    // 4. Otherwise allows
    let bpf_code = [
        ...
        // Check if socket (41 on x86_64)
        sock_filter { code: 0x15, jt: 0, jf: 3, k: 41 },
        // Not socket, allow
        sock_filter { code: 0x06, jt: 0, jf: 0, k: 0x7fff0000 },
        // Load arg0 (domain) - offset 16 in seccomp_data
        sock_filter { code: 0x20, jt: 0, jf: 0, k: 0x00000010 },
        // Check if AF_INET (2)
        sock_filter { code: 0x15, jt: 0, jf: 1, k: 2 },
        // Block: return EPERM
    ...
    eprintln!("Applying BPF filter to block socket(AF_INET)...");
    ...
    eprintln!("Seccomp filter applied successfully (blocks socket(AF_INET), allows execve)");
    Ok(())
}
