//! Sandboxer with WORKING seccomp-BPF filter (based on C example)
...
fn apply_seccomp_block() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // This is the EXACT filter from the working C example
    // It blocks execve (syscall 59 on x86_64)
    let bpf_code = [
