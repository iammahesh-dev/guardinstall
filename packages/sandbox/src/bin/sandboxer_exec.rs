//! Sandboxer using direct execl() after applying seccomp
...
fn main() {
    ...
    if pid == 0 {
        ...
        if let Err(e) = apply_seccomp_block() {
            eprintln!("Failed to apply seccomp: {}", e);
            std::process::exit(1);
        }

        // Run the script using execl directly
        let script_cstr = CString::new(script_path.as_bytes()).unwrap();
        let bash_cstr = CString::new("bash").unwrap();
        let arg0_cstr = CString::new("bash").unwrap();
        let arg1_cstr = CString::new(script_path.as_bytes()).unwrap();
        
        eprintln!("Executing bash with script...");
...
/// Apply seccomp filter that blocks execve
fn apply_seccomp_block() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // BPF program - EXACTLY matching the working C example
    #[repr(C)]
    struct sock_filter {
        code: u16,
        jt: u8,
